...
The subsequent chapter is going to give a detailed view of how the methods described in section \ref{chapter:Concept}, were implemented. 
As the Android-application described in passage \ref{chapter:Integration into existing App} was used as a basis, there is no reasonable alternative to further developing the app with Android Studio. All source code is available at GitHub\footnote{\url{https://github.com/TobiasBauer/androidsigndetection}}.
\section{Development preliminary}
Before the development begins, a phone has to be specified and general informations about native development are given.
...
A tf.Session object encapsulates the environment in which Operation objects are executed, and Tensor objects are evaluated\footnote{\url{https://www.tensorflow.org/api\_docs/python/tf/Session} (visited on 02.14.2018)}. A session owns resources like Variables, in given case, all weights and biases are stored in a session, moreover if they change, say during the process of training, the current values are enclosed by the Session. In order to safe the current state of the graph, Tensorflow provides the Saver class, that allows easy saving of the whole session informations. 
